...
class ChessGame {
  constructor(id, chessServer, gameType, options) {
    ...
    if (options !== undefined && options.depth !== undefined)
      this.depthBlack = options.depth;
    //assumes player always starts game
    this.turnType =
      gameType === GameType.GOLEM_VS_GOLEM ? TurnType.GOLEM : TurnType.PLAYER;
  ...
  performGolemCalculationsWrapper = async (data) => {
    ...
    data.depth =
      data.turnId == PlayerType.WHITE ? this.depthWhite : this.depthBlack;
  ...
  displayGolemCalculationStatus = (success) => {
    if (
      !toBool(
        process.env.LOG_ENABLED_CHESS_GAME_COMPLETED_CALCULATION_WAS_SUCCESSFUL
      )
    )
      return;
  ...
  PerformMoveAndCheckForGameOver = (move) => {
    ...
    if (this.chess.game_over()) {
      ...
      console.log(
        "!!!! game over !!!!! / " + this.moves[this.stepId].winnerType
      );
  ...
  debugLog = (functionName, data) => {
    if (toBool(process.env.LOG_ENABLED_CHESS_GAME_FUNCTION_HEADER))
      console.log(
        `>>>ChessGame::${functionName} ` + JSON.stringify(data, null, 4)
      );
  };
  close = () => {
    this.active = false;
  ...
  getTimeString(timestamp) {
    return moment(new Date().setTime(timestamp /* * 1000*/)).format(
      "YYYY-MM-DD"
    );
  }
  ...
  getGameObject = (isSecure) => {
    let object = {
      ...
      gameStartedTimeString: this.getTimeString(this.gameStartedTime),
       ...
       /* console.log(
        ...
        this.turnId =
          lastMove.turnId === PlayerType.WHITE
            ...
            : PlayerType.WHITE;
        this.turnType =
          lastMove.playerType === TurnType.PLAYER
            ? TurnType.GOLEM
            : TurnType.PLAYER;

        this.globalTurn =
          lastMove.turn === PlayerType.WHITE
            ? PlayerType.BLACK
            : PlayerType.WHITE;
