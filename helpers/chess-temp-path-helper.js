...
class ChessTempPathHelper {
  constructor(gameId, stepId) {
    ...
    this.DefaultFileName = path.join(
      "step_" + stepId.toString().padStart(4, "0") + ".txt"
    );
    ...
    this.ChessBoardFilePath = path.join(
      this.InputFolder,
      "chessboard_" + this.DefaultFileName
    );

    this.OutputLogFilePath = path.join(
      this.OutputFolder,
